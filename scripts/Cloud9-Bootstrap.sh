...
/home/ec2-user/.local/bin/pcluster create-cluster --cluster-name "${CLUSTER_NAME}" --cluster-configuration config.${AWS_REGION_NAME}.yaml --rollback-on-failure false --wait

HEADNODE_PRIVATE_IP=$(/home/ec2-user/.local/bin/pcluster describe-cluster --cluster-name "${CLUSTER_NAME}" | jq -r '.headNode.privateIpAddress')
...
SG_HEADNODE=$(aws cloudformation describe-stack-resources --stack-name "${CLUSTER_NAME}" --logical-resource-id ComputeSecurityGroup --query "StackResources[*].PhysicalResourceId" --output text)
...
if [[ $FSX_ID == "AUTO" ]];then
  FSX_ID=$(aws cloudformation describe-stack-resources --stack-name "${CLUSTER_NAME}" --logical-resource-id FSX0 --query "StackResources[*].PhysicalResourceId" --output text)
fi
